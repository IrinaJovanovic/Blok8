namespace WebApp.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<WebApp.Persistence.ApplicationDbContext>
    {
        ...
        protected override void Seed(WebApp.Persistence.ApplicationDbContext context)
        {
            ...
            if (!context.Pricelists.Any(u => u.ticketType == Enums.TicketType.TimeTicket))
            {
                var price = new Pricelist() { price = 2, ticketType = Enums.TicketType.TimeTicket };
                price.Version = 0;
                context.Pricelists.Add(price);
            }

            if (!context.Pricelists.Any(u => u.ticketType == Enums.TicketType.MonthlyTicket))
            {
                var price = new Pricelist() { price = 79, ticketType = Enums.TicketType.MonthlyTicket };
                price.Version = 0;
                context.Pricelists.Add(price);
            }

            if (!context.Pricelists.Any(u => u.ticketType == Enums.TicketType.AnnualTicket))
            {
                var price = new Pricelist() { price = 750, ticketType = Enums.TicketType.AnnualTicket };
                price.Version = 0;
                context.Pricelists.Add(price);
            }
