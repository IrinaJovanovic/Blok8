...
namespace WebApp.Controllers
{
    public class BuyingTicketController : ApiController
    {
        ...
        public TicketViewModel SetTimeTicketForSale()
        {
            ...
            var price = _unitOfWork.PriceLists.GetAll().Where(u => u.ticketType == TicketType.TimeTicket).Select(u => u.price).FirstOrDefault();
        ...
        public IHttpActionResult BuyTimeTicket()
        {
            ...
            PayPal payPalInfo = new PayPal { CreateTime = create_time, UpdateTime = update_time, TransactionId = id, PayerEmail = payer_email, PayerId = payer_id, Status = status, TicketId = ticket.Id };
            _unitOfWork.PayPals.Add(payPalInfo); //PROVERI 
        ...
        public TicketViewModel SetTicketForSale()
        {
            ...
            var price = _unitOfWork.PriceLists.GetAll().Where(u => u.ticketType == ty).Select(u => u.price).FirstOrDefault()
                - _unitOfWork.PriceLists.GetAll().Where(u => u.ticketType == ty).Select(u => u.price).FirstOrDefault()
        ...
        public IHttpActionResult BuyTicket()
        {
            ...
            PayPal payPalInfo = new PayPal { CreateTime = create_time, UpdateTime = update_time, TransactionId = id, PayerEmail = payer_email, PayerId = payer_id, Status = status, TicketId = ticket.Id };
            _unitOfWork.PayPals.Add(payPalInfo);
        ...
        public IHttpActionResult GetTicetPrice()
        {
            ...
            double price = -1;
            var prices = _unitOfWork.PriceLists.GetAll(); //OVDE 
        ...
        {
            var pricesTemp = _unitOfWork.PriceLists.GetAll(); //OVDE 
